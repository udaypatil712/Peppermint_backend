...
function inObstacle(x, y) {
    const pad = 8;
    return OBSTACLES.some((o) => x > o.x1 - pad && x < o.x2 + pad && y > o.y1 - pad && y < o.y2 + pad);
}
...
class Robot {
    ...
    waypointX = null;
    waypointY = null;
    constructor(id, type, startX, startY, battery, status) {
        this.id = id;
        this.type = type;
    ...
    runStatusMachine(disruptions) {
        ...
        if (this.statusAge < sojourn[this.status])
            return;
        this.statusAge = 0;
        const r = Math.random();
        switch (this.status) {
